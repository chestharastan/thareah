...
const getRoomGroupKey = (room: string) => {
  if (['overview', 'service', 'about', 'chrono', 'portfolio', 'contact'].includes(room)) return 'intro';
  return room;
};
...
export default function App() {
  ...
  const [scrollFadeOpacity, setScrollFadeOpacity] = useState<number>(0);
  const [chronoOpacity, setChronoOpacity] = useState<number>(1);
  const [portfolioOpacity, setPortfolioOpacity] = useState<number>(0);
  const [circleProgress, setCircleProgress] = useState<number>(0);

  const [activeResearchId, setActiveResearchId] = useState<string | null>("diabetes-predict-low-resource");
  const [currentVMDTheme, setCurrentVMDTheme] = useState<string>("ARCH-AI SYNERGY EXPO");
  ...
  const animateScrollTo = (container: HTMLDivElement, targetTop: number, duration: number = 2200, callback?: () => void) => {
  ...
  const runProgrammaticScroll = (targetId: string | null, offsetDelta: number = 0, duration: number = 2000) => {
    ...
    const tryScroll = () => {
      ...
      if (element) {
        animateScrollTo(profileScrollRef.current, element.offsetTop + offsetDelta, duration, () => {
          isProgrammaticScrollRef.current = false;
        });
        return true;
      }
  ...
  const currentMobileScale = isMobile ? Math.min(1, (screenWidth - 20) / 1120) : 1;
  ...
  const handleProfileScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (chronoEl && portfolioEl) {
      ...
      if (scrollTop < triggerStart) {
        ...
        // Chrono fades out in the first 35% of the total transit journey
        const overallProgress = currentRelativeScroll / totalScrollArea;
        const cOpacity = Math.max(0, 1 - (overallProgress / 0.35));
        setChronoOpacity(cOpacity);

        // Portfolio becomes fully active for the clip-path mask to visual
        setPortfolioOpacity(1);
        
        // Disable solid overlay for seamless organic circle zoom
        setScrollFadeOpacity(0);
      }
    } else {
      setChronoOpacity(1);
    ...
    } else if (chronoEl && scrollTop >= chronoEl.offsetTop - clientHeight * 0.45) {
      if (activeRoom !== 'chrono') {
        setActiveRoom('chrono');
      }
    } else if (aboutEl && scrollTop >= aboutEl.offsetTop - clientHeight * 0.45) {
      if (activeRoom !== 'about') {
        setActiveRoom('about');
      }
    } else if (serviceEl && scrollTop >= serviceEl.offsetTop - clientHeight * 0.45) {
  ...
  const navigateToRoom = (room: string, subTab?: string) => {
    ...
    const currentGroup = ['overview', 'service', 'about', 'chrono', 'portfolio', 'contact'].includes(activeRoom) ? 'overview' : (['design', 'webdev', 'ai'].includes(activeRoom) ? 'portfolio' : activeRoom);
    const targetGroup = ['overview', 'service', 'about', 'chrono', 'portfolio', 'contact'].includes(room) ? 'overview' : (['design', 'webdev', 'ai'].includes(room) ? 'portfolio' : room);
    ...
    const isChamberTransition = ['design', 'webdev', 'ai'].includes(activeRoom) || ['design', 'webdev', 'ai'].includes(room);

    if (isChamberTransition) {
      // Direct jump with beautiful scale zoom transition inside Framer Motion
      const isBackToProfile = ['overview', 'service', 'about', 'chrono', 'portfolio', 'contact'].includes(room);
  ...
  const handleWheelScroll = (e: React.WheelEvent) => {
    ...
    if (['overview', 'service', 'about', 'chrono', 'portfolio', 'contact'].includes(activeRoom)) {
      return;
    }
    ...
    if (Math.abs(e.deltaY) > scrollThreshold) {
      ...
      while (currentEl && currentEl !== e.currentTarget && currentEl !== document.body) {
        ...
        if (canScrollY && currentEl.scrollHeight > currentEl.clientHeight + 2) {
          if (dir > 0) {
            ...
            const scrollBottomRemaining = currentEl.scrollHeight - currentEl.clientHeight - currentEl.scrollTop;
      ...
      if (nextIndex >= 0 && nextIndex < pages.length) {
        lastScrollTime.current = now;
        setDirection(dir);
        const targetRoom = pages[nextIndex] === 'overview' && dir < 0 ? 'chrono' : pages[nextIndex];
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (target.closest('.fixed') || target.closest('[data-no-scroll-exit]')) return;
    ...
    switch (activeRoom) {
      ...
      case 'contact':
        return (
          ...
          >
            <div id="profile-section-top" className="w-full min-h-[640px] h-[calc(100vh-53px)] shrink-0 bg-[#fbf9f4]">
              <IntroCollage zoom={1} onNavigateToRoom={navigateToRoom} />
            </div>
            <div id="service-section" className="w-full shrink-0">
              <ServiceSection />
            </div>
            <div id="about-section" className="w-full min-h-[calc(100vh-53px)] bg-white shrink-0 font-sans">
              <AboutMeSection onExploreProjects={(tgt, sub) => navigateToRoom(tgt || 'portfolio', sub)} />
            </div>
            <div 
              id="chrono-section" 
              style={{ opacity: chronoOpacity }}
              className="w-full min-h-[calc(100vh-53px)] bg-white shrink-0 font-sans p-6 md:p-10 relative transition-opacity duration-100 ease-out"
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
                <div className="lg:col-span-5 space-y-3">
                  ...
                  <p className="text-neutral-500 text-xs leading-relaxed max-w-sm">
                    Professional landmarks, research paper hypotheticals, and core capabilities matrices plotted as physical displays.
                  </p>

                  {/* Dynamic Accordion list inside long timeline */}
                  <div id="academic-paper-shelf" className="pt-6 space-y-3 font-medium">
                        ...
                        return (
                          <div 
                            ...
                            className={`p-3 border rounded-xs transition-all ${
                              isActive ? 'bg-neutral-50 border-neutral-900' : 'bg-transparent border-neutral-200 hover:border-neutral-400'
                            }`}
                          >
                            <button
                              onClick={() => setActiveResearchId(isActive ? null : paper.id)}
                              className="w-full text-left flex justify-between items-center text-xs focus:outline-none cursor-pointer"
                            >
                              <span className="font-medium text-neutral-800 pr-4">{paper.title}</span>
                            ...
                            {isActive && (
                              <div className="mt-2.5 pt-2 border-t border-neutral-100 text-[11px] text-neutral-500 space-y-1">
                                <p className="italic font-sans">&ldquo;{paper.question}&rdquo;</p>
                                <p className="leading-snug text-neutral-650 mt-1.5">{paper.abstract}</p>
                              </div>
                            )}
                          </div>
                        );
                ...
                <div className="lg:col-span-7 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {SKILL_CATEGORIES.slice(0, 2).map((cat) => (
                      <div key={cat.id} className="bg-neutral-50 border border-neutral-150 p-4 rounded-xs space-y-4">
                        <div className="space-y-0.5">
                          <span className="font-mono text-[8px] text-amber-600 uppercase font-bold">GROUP PORT</span>
                          <h4 className="font-display font-medium text-xs text-neutral-900">{cat.title}</h4>
                        </div>
                          ...
                          {cat.skills.slice(0, 3).map((s, sIdx) => (
                            <div key={sIdx} className="space-y-1">
                              ...
                              <div className="h-1 bg-neutral-200 rounded-full overflow-hidden">
                                <div className="h-full bg-neutral-900" style={{ width: `${s.level}%` }}></div>
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            >
              ...
              <div className="sticky top-[53px] h-[calc(100vh-53px)] w-full overflow-hidden bg-white flex items-center justify-center">
                ...
                <div 
                  className="absolute inset-0 w-full h-full z-10 origin-center transition-all duration-75"
                  style={{
                    opacity: Math.max(0, 1 - (circleProgress * 1.5)), // fades out early in the transition
                    transform: `scale(${1 - (circleProgress * 0.08)})`, // subtle parallax zoom-out feel
                    pointerEvents: circleProgress < 0.25 ? 'auto' : 'none'
                  }}
                >
                  <RecruitmentBanner onEnterPortals={() => navigateToRoom('portfolio')} isStickyMode={true} />
                </div>
                ...
                <div 
                  ...
                  style={{
                    clipPath: circleProgress === 0 
                      ? 'circle(0% at 50% 50%)' 
                      : circleProgress >= 0.96 
                        ? 'none' 
                        : `circle(${circleProgress * 125}% at 50% 50%)`,
                    opacity: circleProgress === 0 ? 0 : 1,
                    transform: `scale(${0.88 + (circleProgress * 0.12)})`, 
                    pointerEvents: circleProgress > 0.85 ? 'auto' : 'none'
                  }}
                >
                  <PortfolioHub onZoomIn={(chamber) => navigateToRoom(chamber)} />
                </div>

                {/* Outer Glow ring indicating the portal bounds */}
                {circleProgress > 0 && circleProgress < 0.96 && (
                  <div 
                    className="absolute rounded-full border border-amber-500/30 pointer-events-none z-30 transition-shadow duration-75"
                    style={{
                      width: '100px',
                      height: '100px',
                      transform: `scale(${(circleProgress * 2.5 * Math.max(screenWidth, screenHeight)) / 100})`,
                      opacity: Math.max(0, 1.2 - circleProgress),
                      boxShadow: '0 0 40px rgba(217, 119, 6, 0.22), inset 0 0 25px rgba(217, 119, 6, 0.12)',
                    }}
                  />
                )}
              </div>
      ...
      case 'ai':
        return (
          <div className="w-full h-full overflow-hidden">
            <AISystemsHub initialTab={activeSubTab || 'diabetes'} onTabChange={(t) => setActiveSubTab(t)} />
          </div>
        );
      default:
        return null;
